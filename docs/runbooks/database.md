...
Local verification passed against PostgreSQL 16.15, built from the [official source release](https://www.postgresql.org/ftp/source/v16.15/) after SHA-256 verification. Docker was unresponsive, so testing used a temporary cluster with TCP disabled and a private Unix socket. CI is pinned to PostgreSQL 16.15 as well. Fresh migration, prototype adoption, authentication concurrency, tenant isolation, and audit guards are exercised on both databases. Least-privilege PostgreSQL role tests, production backup/restore drills, and deployment readiness remain future work.
